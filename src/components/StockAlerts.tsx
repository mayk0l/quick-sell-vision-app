...
const StockAlerts = ({ stockAlerts, onBack }) => {
  const [alerts, setAlerts] = useState(stockAlerts);

  const sendWhatsAppAlert = (productName) => {
  ...
  const criticalAlerts = alerts.filter(alert => alert.currentStock <= 2);
  const lowStockAlerts = alerts.filter(alert => alert.currentStock > 2 && alert.currentStock < alert.minStock);
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card className="bg-orange-50 border-orange-200">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-2xl font-bold text-orange-700">{lowStockAlerts.length}</p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="space-y-3">
            {criticalAlerts.map((alert, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-200">
                <div>
                  <h4 className="font-semibold text-red-800">{alert.name}</h4>
                ...
                <div className="flex items-center space-x-2">
                  {alert.alertSent ? (
                    ...
                    <Button
                      size="sm"
                      onClick={() => sendWhatsAppAlert(alert.name)}
          ...
          <CardContent className="space-y-3">
            {lowStockAlerts.map((alert, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-orange-50 rounded-lg border border-orange-200">
                <div>
                  <h4 className="font-semibold text-orange-800">{alert.name}</h4>
      ...
      {alerts.length === 0 && (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-green-600 font-medium">¡Stock en perfecto estado!</p>
